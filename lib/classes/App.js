...
module.exports = Class.extend({
  ...
  initChug: function initChug() {
    ...
    ['controllers', 'models'].forEach(function (key) {
    ...
    app.orm = require(app.config.orm);
    app.dbs = {};
    for (var dbKey in app.config.dbs) {
      var Database = require(app.config.orm);
      ...
      db
        ...
        .complete(function (err) {
          if (!!err) {
            app.logger.error('Unable to connect to "' + dbKey + '" database.');
          } else {
            app.logger.info('Connected to "' + dbKey + '" database.');
          }
        });
      app.dbs[dbKey] = db;
