...
module.exports = Class.extend({
  ...
  init: function (options) {
    ...
    app.config = {
      ...
      logger: [
        ...
        {transport: 'file', level: 'info', pattern: 'log/${YYYY}/${MM}/${DD}/lighterio-${HOST}.log'}
      ],
      asciiArt: null,
      cullEnv: true,
      enableCache: true,
    ...
    // If running from another directory, enter the desired directory.
    if (app.config.dir != cwd) {
      try {
  ...
  initLogger: function () {
    ...
    if (logger instanceof Array) {
      logger = app.logger = require('cedar')(logger);
    }
    else {
      try {
        ...
        console.error('[Lighter] Custom loggers must have "debug", "trace", "log", "info" and "warn" methods.');
        process.exit();
      }
    }
  },
  ...
  initChug: function () {
    ...
    ['models', 'controllers'].forEach(function (key) {
      var Parent = require('./' + capitalize(key).substr(0, key.length - 1));
      var namePattern = /^.*[\/\\]([^\/\\]+)\.[^\/\\]+$/;
