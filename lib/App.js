...
module.exports = Class.extend({
  ...
  init: function init(options) {
    ...
    app.config = {
      env: env,
      dir: cwd,
      overridesPath: cwd + '/overrides.json',
      logsDir: 'logs',
      server: null,
      hostName: ipAddress || '127.0.0.1',
      httpPort: 8888,
      ...
      cache: null,
      logger: null,
      logLevel: ((env == 'canary' || env == 'prod') ? 'info' : (env == 'debug' ? 'debug' : 'log')),
      asciiArt: null,
      cullEnv: true,
      enableCache: true,
      enableChug: true,
      ...
      watchSplode: false,
      watchZa: false,
      watchIgnoreDirPattern: /^(\..*|logs|controllers|coverage|node_modules|public|scripts|styles|test|uploads|views)$/,
      watchIgnoreFilePattern : /^(\.DS_Store)$/,
    ...
    app.decorateConfig(app.requireJson(app.config.overridesPath));
  ...
  requireJson: function requireJson(path) {
    ...
    try {
      ...
      console.warn(red + bold + e);
      return {};
    }
  },

  decorateConfig: function decorateConfig(decorations) {
    var app = this;
  ...
  initLogger: function initLogger() {
    ...
    if (!app.logger) {
      ...
      if (!app.isDev) {
        app.logger = require('cedar')('file', {path: app.config.logsDir+"/cedar.log"});
      }
      else{
  ...
  initChug: function initChug() {
    ...
    app.chug.onceReady(function () {
      ...
      if (app.isDev) {
        ...
        app.watchAndExit('.', app.config.watchIgnoreDirPattern, app.config.watchIgnoreFilePattern);
  ...
  watchAndExit: function watchAndExit(dir, ignoreDirPattern, ignoreFilePattern) {
    var watchCount = 0;
    try {
      watchCount++;
      if (watchCount < 2e2) {
        fs.watch(dir, function (e, filename) {
          if(ignoreFilePattern.test(filename)){
            return;
          }
    ...
    fs.readdir(dir, function (err, files) {
      ...
      files.forEach(function (file) {
        if (!ignoreDirPattern.test(file)) {
          var path = dir + '/' + file;
          fs.stat(path, function (err, stat) {
            if (stat.isDirectory()) {
              watchAndExit(path, ignoreDirPattern, ignoreFilePattern);
            }
          });
        }
      });
