...
module.exports = Flagger.extend({
  ...
  configure: function () {
    ...
    // Auto-install required modules in a dev environment.
    if (self.isDev) {
      try {
        require('requiry')
      } catch (ignore) {
  ...
  initServer: function () {
    ...
    if (self.isWorker) {
      ...
      var apiMethods = ['use', 'get', 'post']
      apiMethods.forEach(function (methodName) {
      ...
      if (self.maxMiddlewareQueueSize && self.server.unuse) {
        var queue = []
  ...
  initRings: function () {
    var self = this
    var omen = self.omen = require('omen')
  ...
  initChug: function () {
    ...
    var chug = self.chug = require('chug')
  ...
  loadApi: function () {
    var self = this
    var app = self
    var Model = self.model = require(dir + '/lib/model')
    var namePattern = /^.*[\/\\]([^\/\\]+)\.[^\/\\]+$/
    var load = self.models = self.chug(self.models)
      .require(function (Type) {
        var asset = this
        if (!Type.extend) {
          Type = asset.module = Model.extend(Type)
        }
        var instance = Type.instance = new Type(app, asset)
        if (!instance.app) {
          Model.call(instance, app, asset)
        }
  ...
  loadUi: function () {
    ...
    self.uiAssets = chug()
    self.uiLoads = []
    var dirs = ['publics', 'scripts', 'styles', 'views']
    ...
    chug.then(function () {
      ...
      self.uiAssets
        ...
        .then(function () {
          ...
          js = js.replace(/['"]CSS_TEXT['"]/g, JSON.stringify(css))
  ...
  finishLoading: function () {
    ...
    chug.then(function () {
      ...
      self.models.each(function (asset) {
        var instance = asset.module.instance
      ...
      if (self.isDev) {
        ...
        watchStream.on('event', function (event) {
          var type = event.type
          var path = event.path
          var change = (type === 'change')
          var ignore = /\/(m\.html|build)($|\/)/.test(path)
          if (change && !ignore) {
            var asset = chug.cache.get(path)
            self.log.warn('[Lighter] Changed ' + shorten(path).cyan + '.')
            // TODO: Partial reloading.
            if (asset && false) {
              asset.parents.forEach(function (load) {
                load.handleChange(path)
              })
              // Tell web clients to reload.
              self.beams.each(function (client) {
  ...
  initPorta: function () {
    var self = this
    var porta = self.porta = require('porta')
  ...
  initMimo: function () {
    var self = this
    var mimo = self.mimo = require('mimo')
    self.addAssets('scripts', mimo.jymin)
  },
  ...
  initBeams: function () {
    var self = this
    var beams = self.beams = require('beams')({
      log: self.log,
      server: self.server
    })
