...
module.exports = Flagger.extend({
  ...
  decorateState: function (state, request, response) {
    ...
    if (/mobile/i.test(request.headers['user-agent'])) {
      state.device = 'mobile';
    }
    else {
    ...
    chug.onceReady(function () {
      ...
      self.uiAssets
        ...
        .then(function () {
          ...
          var url = '/a.js';
          script.route(url);
          self.log.info('[Lighter] UI JS routed to ' + url.cyan + '.');
          self.ui = js;
        });
    });
  },
  ...
  /**
   * Set up Einstein as an isomorphic transport layer for views and data.
   */
  initEinstein: function () {
    var self = this;
    var einstein = require(self.modulesDir + '/einstein/einstein');
    self.expose('Einstein', einstein);
    self.addAssets('scripts', einstein.jymin);

    // Require einstein later so minification can begin before it uses chug.onReady.
    setImmediate(function () {
      einstein(self);
    });
  },
