...
module.exports = Flagger.extend({
  ...
  loadApi: function () {
    var self = this
    var app = self
    var Model = require(dir + '/lib/model')
    self.expose('model', Model)
    var namePattern = /^.*[\/\\]([^\/\\]+)\.[^\/\\]+$/
    var key = 'models'
  ...
  initMimo: function () {
    var self = this
    var mimo = require('mimo')
    self.expose('Mimo', mimo)
  ...
  initBeams: function () {
    ...
    // In dev mode, let Beams listen for changes.
    if (self.isDev) {
