          ...
          if (typeof controller[property] == 'function') {
            (function (action) {
              if (/(GET|PUT|POST|DELETE)/.test(action.name)) {
                ...
                if (typeof settings.formatControllerPath == 'function') {
                  url = settings.formatControllerPath(url);
                }
      ...
      for (var dbKey in settings.dbs) {
          ...
          db
            ...
            .complete(function (err) {
              if (!!err) {
                log.error('Unable to connect to db: "' + dbKey + '"');
              } else {
                log.info('Connected to db: "' + dbKey + '"');
              }
            });
